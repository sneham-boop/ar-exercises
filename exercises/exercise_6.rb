...
# identifies which store each employee belongs to. It points to the `id` (integer) column of their 
# store.
...
# create stores): `@store1.employees.create(first_name: "Khurram", last_name: "Virani", 
# hourly_rate: 60)`
# 4. Go ahead and create some more employees using the create method. You can do this by 
#   making multiple calls to create (like you have before.) No need to assign the employees to 
#   variables though. Create them through the `@store#` instance variables that you defined in 
#   previous exercises. Create a bunch under `@store1` (Burnaby) and `@store2` (Richmond). 
